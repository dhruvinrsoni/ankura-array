/* Yatra-Monitor — PDF ticket parser + grid renderer
...
(function(){
  ...
  var State = _fw.State;
  var tickets = State.load('yatra_tickets', []);
  // Migrate legacy global storage (if user previously saved without instance namespacing)
  try{
    var legacy = localStorage.getItem('yatra_tickets');
    if(legacy && (!tickets || tickets.length===0)){
      try{ var parsed = JSON.parse(legacy); if(Array.isArray(parsed) && parsed.length>0){ tickets = parsed; State.save('yatra_tickets', tickets); localStorage.removeItem('yatra_tickets'); } }catch(e){}
    }
  }catch(e){}
  // IndexedDB helpers for large PDF storage
  var DB_NAME = 'YatraMonitorDB';
  var DB_STORE = 'pdfs';
  var db = null;
  ...
  function deleteAll(){
    ...
    tickets=[]; logs=[];
    try{ State.clear('yatra_tickets'); localStorage.removeItem(LOG_KEY); }catch(e){}
  ...
  function saveTicket(parsed){
    ...
    try{ State.save('yatra_tickets', tickets); } catch(e){ log('Save failed: '+e.message,'err'); }
  }
  ...
  function deleteTicket(id){
    ...
    try{ State.save('yatra_tickets', tickets); } catch(e){}
    renderGrid();
  }
  ...
  window.YATRA_DEBUG = { tickets: tickets, sessionBlobs: sessionBlobs };
